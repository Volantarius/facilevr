...
drive.Register( "drive_physical_two",
{
	--
	-- Called on creation
	--
	...
	CalcView = function( self, view )
		...
		self:CalcView_ThirdPerson( view, idealdist, 2, { self.Entity, self.Player } )
	...
	Move = function( self, mv )
		...
		--[[local speed_forward = mv:GetForwardSpeed()
		local speed_side = mv:GetSideSpeed()
		local speed_up = mv:GetUpSpeed()]]
		...
		--[[if ( speed_forward > 0 ) then
			normal_forwad = 1
		elseif ( speed_forward < 0 ) then
			normal_forwad = -1
		end
		
		if ( speed_side > 0 ) then
			normal_side = 1
		elseif ( speed_side < 0 ) then
			normal_side = -1
		end]]
		...
		--local move_speed_test = math.Clamp( math.abs( speed_forward ) + math.abs( speed_side ), 0, 1 )
		...
		--
		--vel = vel + (forward_clamped * speed_forward * speed)
		--vel = vel + (right_clamped   * speed_side    * speed)
		--vel = vel + (ang:Up()		 * speed_up      * speed)
	...
	FinishMove = function( self, mv )
		...
		if ( not self.Player:Alive() ) then
			...
			self.Player:SetPos( self.Entity:GetPos() + Vector(0, 0, -32) )
		end
		...
		--
		if ( SERVER ) then
			if ( not IsValid( self.Entity:GetPhysicsObject() ) ) then
				self:Stop()
				return
			end
			
			self.Entity:SetPhysicsAttacker( self.Player, 5 )
