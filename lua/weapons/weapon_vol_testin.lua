...
function SWEP:Think()
	...
	if ( self.Owner:KeyDown( IN_ATTACK ) ) then
		local cancelVelocity = self.Owner:GetVelocity() * -1
		...
		if (fartPoint) then
			...
			local newPos = fartPoint - ( fartDist * self.Owner:GetAimVector() )
			
			newVelocity = newPos - oldPos
			newVelocity = 10 * newVelocity
		end
		
		self.Owner:SetVelocity( cancelVelocity + newVelocity )
	end
