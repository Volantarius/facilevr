...
SWEP.Primary.DefaultClip 	= 30-- Only for this tool
...
if CLIENT then
	SWEP.PrintName 	= "(TOOL) DESTROY"
	SWEP.ToolName = "Destroy Tool"
	SWEP.Description = "Get rekt."
	--SWEP.info = [[Destroy objects!]]
	SWEP.info_left = "Destroy an object!"
	
	killicon.Add( "weapon_vdm_tool_destroy", "killicons/swep_default", Color(255, 255, 255, 255) )
...
function SWEP:PrimaryAttack()
	...
	-- Same logic, only remove ammo on real hits
	if ( good ) then
...
function SWEP:do_remove( tr, predicted )
	...
	if ( SERVER ) then
		...
		dmg:SetDamageForce( tr.Normal * 3400 )
