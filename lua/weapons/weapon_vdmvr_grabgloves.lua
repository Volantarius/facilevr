...
function SWEP:Think()
	...
	if ( holding_entity ) then
		...
		if ( IsValid(e_physics) ) then
			...
			--[[
			local pinfo_fire_ang = self:GetInfo( "favr_fire_ang" )
			
			if ( pinfo_fire_ang ) then
				fire_ang = Angle( pinfo_fire_ang )
				
				return fire_ang
			end
			
			SERVER is player.vr_mode
			]]
			
			-- Try to angle the object to match our angles we want
			--[[local phys_forward = phys_ang:Forward()
			local fart_test = aim_vector - phys_forward
			local bbb = aim_vector * 1.1
			local farrr = aim_vector:Dot( phys_forward )
			fart_test = fart_test * farrr]]
			
			--output_velocity_ang = fart_test * ( output_velocity_ang:Dot( fart_test ) )
			
			-- Super freaking nice, smooth as butter
			e_physics:SetAngleVelocityInstantaneous( output_velocity_ang )
