...
list.Set( "FacileDesktopWindows", "PlayerEditor", {
	...
	init		= function( icon, window )
		...
		for name, model in SortedPairs( player_manager.AllValidModels() ) do
			...
			PanelSelect:AddPanel( icon, { cl_facile_playermodel = name } )
		...
		local trailcontrols = window:Add( "DPanel" )
		trailcontrols:DockPadding( 8, 8, 8, 8 )
		
		local trailtab = sheet:AddSheet( "Trails", trailcontrols, "icon16/rainbow.png" )
		
		local trailSelect = trailcontrols:Add( "MatSelect" )
		trailSelect:Dock( TOP )
		trailSelect:SetConVar( "cl_facile_trail" )
		
		local trailMats = file.Find( "materials/trails/*.vmt", "GAME" )
		
		for k,name in ipairs( trailMats ) do
			local matname = string.gsub( name, "%.vmt$", "" )
			
			trailSelect:AddMaterial( matname, "trails/"..matname )
		end
		
		trailMats = nil
		
		local trailEnable = trailcontrols:Add( "DCheckBoxLabel" )
		trailEnable:SetConVar( "cl_facile_trail_enabled" )
		trailEnable:Dock( TOP )
		trailEnable:SetTall( 50 )
		trailEnable:SetDark( true )
		trailEnable:SetText( "Enabled" )
		
		local trailAdditive = trailcontrols:Add( "DCheckBoxLabel" )
		trailAdditive:SetConVar( "cl_facile_trail_add" )
		trailAdditive:Dock( TOP )
		trailAdditive:SetTall( 50 )
		trailAdditive:SetDark( true )
		trailAdditive:SetText( "Additive" )
		
		local trailColor = trailcontrols:Add( "DColorMixer" )
		trailColor:SetAlphaBar( false )
		trailColor:SetPalette( false )
		trailColor:Dock( TOP )
		trailColor:SetSize( 200, math.min( window:GetTall() / 3, 260 ) )
		trailColor:SetVector( Vector( GetConVarString( "cl_facile_trail_color" ) ) )
		trailColor.ValueChanged = function()

			RunConsoleCommand( "cl_facile_trail_color", tostring( trailColor:GetVector() ) )

		end
		
		local trailWidthStart = trailcontrols:Add( "DNumSlider" )
		trailWidthStart:SetConVar( "cl_facile_trail_start" )
		trailWidthStart:Dock( TOP )
		trailWidthStart:SetTall( 50 )
		trailWidthStart:SetDark( true )
		trailWidthStart:SetDecimals( 0 )
		trailWidthStart:SetMinMax( 2, 24 )
		trailWidthStart:SetText( "Start Width" )
		
		local trailWidthEnd = trailcontrols:Add( "DNumSlider" )
		trailWidthEnd:SetConVar( "cl_facile_trail_end" )
		trailWidthEnd:Dock( TOP )
		trailWidthEnd:SetTall( 50 )
		trailWidthEnd:SetDark( true )
		trailWidthEnd:SetDecimals( 0 )
		trailWidthEnd:SetMinMax( 0, 32 )
		trailWidthEnd:SetText( "End Width" )
		...
		local function UpdateBodyGroups( pnl, val )
			if ( pnl.type == "bgroup" ) then
				...
				local str = string.Explode( " ", GetConVarString( "cl_facile_playerbodygroups" ) )
				if ( #str < pnl.typenum + 1 ) then for i = 1, pnl.typenum + 1 do str[ i ] = str[ i ] or 0 end end
				str[ pnl.typenum + 1 ] = math.Round( val )
				RunConsoleCommand( "cl_facile_playerbodygroups", table.concat( str, " " ) )

			elseif ( pnl.type == "skin" ) then

				mdl.Entity:SetSkin( math.Round( val ) )
				RunConsoleCommand( "cl_facile_playerskin", math.Round( val ) )
		...
		local function RebuildBodygroupTab()
			...
			if ( nskins > 0 ) then
				...
				skins:SetDecimals( 0 )
				skins:SetMax( nskins )
				skins:SetValue( GetConVarNumber( "cl_facile_playerskin" ) )
				skins.type = "skin"
			...
			local groups = string.Explode( " ", GetConVarString( "cl_facile_playerbodygroups" ) )
		...
		local function UpdateFromConvars()
			...
			local model = LocalPlayer():GetInfo( "cl_facile_playermodel" )
			local modelname = player_manager.TranslatePlayerModel( model )
			util.PrecacheModel( modelname )
			mdl:SetModel( modelname )
			mdl.Entity.GetPlayerColor = function() return Vector( GetConVarString( "cl_facile_playercolor" ) ) end
		...
		local function UpdateFromControls()
			...
			RunConsoleCommand( "cl_facile_playercolor", tostring( plycol:GetVector() ) )
			RunConsoleCommand( "cl_facile_weaponcolor", tostring( wepcol:GetVector() ) )
